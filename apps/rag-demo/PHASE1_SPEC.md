...
- Dependency target moved to `namel3ss==0.1.0a18`.
...
- `namel3ss==0.1.0a18`.
- Python runtime `>=3.14` (required by `namel3ss-0.1.0a18` wheel metadata).
- Local `n3` commands executed from project venv where possible.

Dependency acceptance checks:

1. `python -m namel3ss --version` returns `namel3ss 0.1.0a18`.
...
13. Project dependency target is `namel3ss==0.1.0a18` and runtime checks pass in a Python 3.14 environment.
...
1. Land dependency/runtime baseline (`0.1.0a18`, Python 3.14).
